...
// 3var.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
int countK = 0, c_b = 0;
int CountTopK(Tree* t); //Подсчет вершин с данными k
...
void main()
{
	...
	for (;;)
	{
		...
		case 8:
			countK = 0;
			cout << "Количество вершин: " << CountTopK(Root) << " четных ключей: ";
			cout << countK << endl;
			break;
		case 9:  exit(0);
		}
...
int CountTopK(Tree* Root)
{
	...
	return 1 + CountTopK(Root->Left) + CountTopK(Root->Right);
}